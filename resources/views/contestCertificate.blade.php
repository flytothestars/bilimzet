	...
	<div>
		<form id="form" method="post" action="{{ route(($certId ? 'admin.updateCertificate' : 'admin.storeCertificate'), [ 'id' => $id, 'certId' => $certId ]) }}" enctype="multipart/form-data">
			...
			<div class="form-group">
				...
				<input value="{{ $item->course_title ?? old('course_title') }}" class="form-control @error('course_title') is-invalid @enderror"
						 type="text" name="course_title" id="course_title" placeholder="">
				@error('course_title')
			...
			<div class="cert" style="line-height:1;">
				...
				<div class="name-wrap">
					...
					<input type="text" class="name-inp" value="{{ $item->fio ?? old('fio') }}" name="fio" id="fio">
				...
				<div class="accept-wrap">
					...
					<input type="text" class="accept-inp" value="{{ $item->title ?? old('title') }}" name="title" id="title">
					<div class="accept-hint">прошел(ла) курс повышения квалификации на тему / тақырыбында</div>
				</div>
				<div class="clock-wrap">
					<input type="text" class="clock-inp" value="{{ $item->duration ?? old('duration') }}" name="duration" id="duration">
				...
				<div class="foot">
					<div class="foot-date">
						...
						<div class="foot-wrap">
					<span>“<input type="text" value="{{ $item->day ?? old('day') }}" name="day" id="day" class="foot-inp foot-inp_d">”</span>
							<input type="text" мalue="{{ $item->month ?? old('month') }}" name="month" id="month" class="foot-inp foot-inp_m">
					...
					<div class="foot-reg">
						...
						<div>
							Регистрационный №
							<input type="text" value="{{ $item->month ?? old('month') }}" name="reg_number" id="month" class="sign-inp">
						</div>
					</div>
				</div>

				<input value="{{ $item->year ?? old('year') }}" type="hidden" name="year" id="year">
